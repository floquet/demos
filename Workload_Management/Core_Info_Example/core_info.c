...
void print_core_info(MPI_Comm comm)
{
    int rank, thread;
    cpu_set_t coremask;
    ...
    # pragma omp parallel private(thread, coremask, core)
    {
#ifdef OMP
        ...
        # pragma omp barrier
#ifdef OMP
        printf("rank %2d, thread %2d, host %s, core %s\n", rank, thread, hostname, core);
#else
        printf("rank %2d, host %s, core %s\n", rank, hostname, core);
#endif
    }
}

void print_core_info_(MPI_Comm *comm) { print_core_info(*comm); return; }
