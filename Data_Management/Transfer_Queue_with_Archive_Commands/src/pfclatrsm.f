      SUBROUTINE PFCLATRSM( SIDE, UPLO, TRANS, DIAG, IODEV, M, N, IC,
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, BLACS_PINFO, CLAREAD,
     $                   DESCINIT, LAIO_INFO, PCGEMM, PCSCAL, PCTRSM,
     $                   PFMAXSIZE, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( DESCB( CTXT_ ), 'PFCLATRSM', 16 )
         ENDIF
         RETURN
      ENDIF
...
* 1:jend
     ...
     $                  LDA, INFO )
         ...
         MM = ISIZE
         NN = JSIZE
         CALL CLAREAD( IODEV, ISIZE, JSIZE, ICPOS, JCPOS,
...
*
            ...
            IF( HASWORK ) THEN
               IF( ISLEFT ) THEN
                  CALL PCGEMM( TRANSA, 'N', MM, NN, KK, LALPHA,
