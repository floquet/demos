      SUBROUTINE PFSGETF2( M, N, CWORK, IC, JC, DESCC, IPIV, INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   PSGEMM, PSGETRF, PSLAPIV, PSTRSM, PXERBLA,
     $                   SLAREAD, SLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*     .. Executable Statements ..
     ...
     $                     MYPCOL )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCC( CTXT_ ), 'PFSGETF2', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
      INFO = 0
      CALL SLAREAD( IODEV, M, NCOLB, IC, JC, CWORK( IP_BMAT ), 1, 1,
     $              DESCB, INFO )
...
*
      IINFO = 0
      CALL PSGETRF( M, NCOLB, CWORK( IP_BMAT ), 1, 1, DESCB,
     $              IPIV( IP_BPIV ), IINFO )
      IF( IINFO.GT.0 ) THEN
*
...
*
      INFO = 0
      CALL SLAWRITE( IODEV, M, NCOLB, IC, JC, CWORK( IP_BMAT ), 1, 1,
     $               DESCB, INFO )
...
*
      ...
      IF( ANEED+BNEED.GT.LWORK ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICONTXT, 'PFSGETF2', 6 )
         ENDIF
         LASTINDEX = ( JSTARTB-1 )
         GOTO 90
      ENDIF
...
*
         IIC = ( IC-1 ) + IC
         JJC = ( JC-1 ) + JSTARTB
         CALL SLAREAD( IODEV, M, JSIZEB, IIC, JJC, CWORK( IP_BMAT ), 1,
     $                 1, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAREAD returns info = ',
     $                INFO )
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PSLAPIV( 'Forward', 'Rowise', 'Columwise', MM, NN,
...
*
            ...
            IA = ISTARTA
            JA = 1
            CALL SLAREAD( IODEV, ISIZEA, JSIZEA, IIC, JJC,
...
*
               ...
               HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
               IF( HASWORK ) THEN
                  CALL PSLAPIV( 'Forward', 'Rowwise', 'Columnwise', MM,
...
*
                  ...
                  HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
                  IF( HASWORK ) THEN
                     CALL PSLAPIV( 'Forward', 'Rowwise', 'Columnwise',
...
*
                  ...
                  HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
                  IF( HASWORK ) THEN
                     CALL PSTRSM( 'Leftside', 'LowerTriangular',
     ...
     $                      ( KK.GE.1 )
                  IF( HASWORK ) THEN
                     ALPHA = -ONE
                     BETA = ONE
                     CALL PSGEMM( 'Notrans', 'NoTrans', MM, NN, KK,
     $                            ALPHA, CWORK( IP_AMAT ), IA, JA,
     $                            DESCA, CWORK( IP_BMAT ), IB, JB,
...
*
               ...
               IF( HASWORK ) THEN
                  ALPHA = -ONE
                  BETA = ONE
                  CALL PSGEMM( 'Notrans', 'NoTrans', MM, NN, KK, ALPHA,
...
*
         ...
         IF( HASWORK ) THEN
            ...
            IB = JSTARTB
            JB = 1
            CALL PSGETRF( MM, NN, CWORK( IP_BMAT ), IB, JB, DESCB,
     $                    IPIV( IP_BPIV ), IINFO )
            IF( IINFO.GT.0 ) THEN
*
...
*
         IIC = ( IC-1 ) + IC
         JJC = ( JC-1 ) + JSTARTB
         CALL SLAWRITE( IODEV, M, JSIZEB, IIC, JJC, CWORK( IP_BMAT ), 1,
     $                  1, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAWRITE returns info = '
     $                , INFO )
         JSTARTB = JENDB + 1
   ...
   70 CONTINUE
      IF( JSTARTB.LE.MINMN ) THEN
         ...
         IF( HASWORK ) THEN
            ...
            MM = M - ISTARTB + 1
            NN = JSIZEB
            CALL SLAREAD( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ),
     ...
     $                    IPIV( IP_BPIV ), IB, 1, DESCIPIV, IDUMMY )
            MM = M - ISTARTB + 1
            NN = JSIZEB
            CALL SLAWRITE( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ),
     $                     IB, JB, DESCB, INFO )
            CALL ASSERT( INFO.EQ.0,
...
*
     ...
     $               DESCB( CTXT_ ), LDPIV, INFO )
      ...
      IF( HASWORK ) THEN
         ...
         JB = 1
         MM = M - ISTARTB + 1
         CALL SLAREAD( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ), IB,
     $                 JB, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAREAD returns info = ',
     $                INFO )
         MM = MIN( M, N ) - ISTARTB + 1
     ...
     $                 IB, 1, DESCIPIV, IDUMMY )
         MM = M - ISTARTB + 1
         CALL SLAWRITE( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ), IB,
     $                  JB, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAWRITE returns info = '
     $                , INFO )
      ENDIF
