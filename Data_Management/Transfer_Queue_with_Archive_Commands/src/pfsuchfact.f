      SUBROUTINE PFSUCHFACT( IODEV, M, IC, JC, WORK, LWORK, INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   PFMAXSIZE, PSGEMM, PSPOTRF, PSSYRK, PSTRSM,
     $                   PXERBLA, SLAREAD, SLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICONTXT, 'PFSUCHFACT', -INFO )
         ENDIF
         RETURN
      ENDIF
*
...
*
      ...
      IIC = ( IC-1 ) + ISTARTB
      JJC = ( JC-1 ) + JSTARTB
      CALL SLAREAD( IODEV, ISIZEB, JSIZEB, IIC, JJC, WORK( IP_BMAT ), 1,
     $              1, DESCB, INFO )
...
*
         LINFO = 0
         CALL PSPOTRF( 'Upper', ISIZEB, WORK( IP_BMAT ), 1, 1, DESCB,
     $                 LINFO )
         IF( LINFO.GE.1 ) THEN
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PSTRSM( 'Leftside', 'UpperTriangular', 'T',
...
*
            LINFO = 0
            CALL PSPOTRF( 'Upper', ISIZE, WORK( IP_BMAT ), ISTART,
     $                    ISTART, DESCB, LINFO )
            IF( LINFO.GE.1 ) THEN
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PSSYRK( 'Upper', 'T', NN, KK, ALPHA,
     $                      WORK( IP_BMAT ), ISTART, ( IEND+1 ), DESCB,
...
*
            ...
            IF( HASWORK ) THEN
               CALL PSGEMM( 'T', 'NoTranspose', MM, NN, KK, ALPHA,
     $                      WORK( IP_BMAT ), ISTART, ( IEND+1 ), DESCB,
...
* end do istart
      ...
      IIC = ( IC-1 ) + ISTARTB
      JJC = ( JC-1 ) + JSTARTB
      CALL SLAWRITE( IODEV, ISIZEB, JSIZEB, IIC, JJC, WORK( IP_BMAT ),
     $               1, 1, DESCB, INFO )
...
*
         ...
         IIC = ( IC-1 ) + ISTARTB
         JJC = ( JC-1 ) + JSTARTB
         CALL SLAREAD( IODEV, ISIZEB, JSIZEB, IIC, JJC, WORK( IP_BMAT ),
     $                 1, 1, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '** PFxUCHFACT: LAREAD returns info = '
     $                , INFO )
         ISTARTA = 1
...
*
            ...
            IIC = ( IC-1 ) + ISTARTA
            JJC = ( JC-1 ) + JSTARTB
            CALL SLAREAD( IODEV, ISIZEA, JSIZEB, IIC, JJC,
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PSTRSM( 'Leftside', 'UpperPart', 'T',
     $                      'NonunitDiagonal', MM, NN, ALPHA,
     $                      WORK( IP_BMAT ), 1, 1, DESCB,
...
*
         ...
         IIC = ( IC-1 ) + ISTARTB
         JJC = ( JC-1 ) + JSTARTB
         CALL SLAWRITE( IODEV, ISIZEB, JSIZEB, IIC, JJC,
