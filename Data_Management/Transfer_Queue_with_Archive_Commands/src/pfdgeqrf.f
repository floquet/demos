      SUBROUTINE PFDGEQRF( M, N, A, IA, JA, DESCA, TAU, WORK, LWORK,
     $                     INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   LAIO_INFO, PCHK1MAT, PFDQRFACT2, PFMAXSIZE,
     $                   PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*     .. Executable Statements ..
     ...
     $                     MYPCOL )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFDGEQRF', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
...
* need more storage to proceed effectively
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( DESCA( CTXT_ ), 'PFDGEQRF', 9 )
         ENDIF
         RETURN
      ENDIF
     ...
     $               MAX( 1, LOCP ), INFO )
      CALL ASSERT( INFO.EQ.0, '** PFGEQRF: descinit returns info ',
     $             INFO )
      CALL PFDQRFACT2( IODEV, M, N, A( 1 ), IA, JA, DESCA, TAU, WORK,
     $                 LWORK )
      WORK( 1 ) = INEED
      A( 1 ) = ANEED
      INFO = 0
