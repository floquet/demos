      SUBROUTINE PFCQRFACT2( IODEV, M, N, CWORK, IC, JC, DESCC, TAUC,
     $                       WORK, LWORK )
*
*
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, CFILL, CGEBR2D,
     $                   CGEBS2D, CLAREAD, CLAWRITE, DESCINIT,
     $                   LAIO_INFO, PCGEQRF, PCUNMQR, PFCCOPYTAU,
     $                   PFMAXSIZE
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          CMPLX, INT, MAX, MIN, REAL
*     ..
...
*
     ...
     $             INFO )
      ICPOS = ( IC-1 ) + 1
      JCPOS = ( JC-1 ) + 1
      CALL CLAREAD( IODEV, M, JSIZEB, ICPOS, JCPOS, CWORK( IP_BMAT ), 1,
     $              1, DESCB, INFO )
      CALL ASSERT( INFO.EQ.0, 'PFxQRFACT2 : LAREAD returns info ',
     $             INFO )
      LINFO = 0
      CALL PCGEQRF( M, JSIZEB, CWORK( IP_BMAT ), 1, 1, DESCB,
...
*
      ...
      IF( .NOT.ALL_INVOLVED ) THEN
         ...
         IF( HASWORK ) THEN
            IF( MYPROW.EQ.DESCB( RSRC_ ) ) THEN
               CALL CGEBS2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ, 1,
     $                       WORK( IP_TAUB ), LOCQ )
            ELSE
               CALL CGEBR2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ, 1,
     ...
     $                       MYPCOL )
      ...
      ENDIF
      JCPOS = ( JC-1 ) + JSTARTB
      CALL PFCCOPYTAU( JSIZEB, 1, WORK( IP_TAUB ), DESCB, JCPOS, TAUC,
     $                 DESCC )
      ...
      ICPOS = ( IC-1 ) + 1
      JCPOS = ( JC-1 ) + 1
      CALL CLAWRITE( IODEV, M, JSIZEB, ICPOS, JCPOS, CWORK( IP_BMAT ),
     $               1, 1, DESCB, INFO )
...
*
         ...
         JCPOS = ( JC-1 ) + JSTARTB
         INFO = 0
         CALL CLAREAD( IODEV, M, JSIZEB, ICPOS, JCPOS, CWORK( IP_BMAT ),
     $                 1, 1, DESCB, INFO )
...
*
     ...
     $                   JSTARTA )
            ...
            INFO = 0
            CALL CLAREAD( IODEV, ISIZEA, JSIZEA, ICPOS, JCPOS,
     $                    CWORK( IP_AMAT ), IAPOS, JAPOS, DESCA, INFO )
            CALL ASSERT( INFO.EQ.0,
     $                   'PFxQRFACT2 : laread for A returns info ',
     $                   INFO )
            CALL CFILL( TAUA_DIM, DZERO, WORK( IP_TAUA ), 1 )
            CALL PFCCOPYTAU( JSIZEA, JCPOS, TAUC, DESCC, JAPOS,
     $                       WORK( IP_TAUA ), DESCA )
*
...
*
     ...
     $                ( JSIZEB.GE.1 )
            IF( HASWORK ) THEN
               ...
               JBPOS = 1
               LINFO = 0
               CALL PCUNMQR( 'Leftside', 'C', MM, NN, KK,
...
*
         ...
         IF( HASWORK ) THEN
            ...
            JBPOS = 1
            LINFO = 0
            CALL PCGEQRF( MM, NN, CWORK( IP_BMAT ), IBPOS, JBPOS, DESCB,
     $                    WORK( IP_TAUB ), WORK( IFREE ), NFREE, LINFO )
            QR_NEED = MAX( QR_NEED, INT( WORK( IFREE ) ) )
            IF( LINFO.NE.0 ) THEN
               IF( LINFO.GE.1 ) THEN
...
*
     ...
     $                    NPROW )
                  IF( MYPROW.EQ.IROWB ) THEN
                     CALL CGEBS2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ,
     $                             1, WORK( IP_TAUB ), LOCQ )
                  ELSE
                     CALL CGEBR2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ,
     ...
     $                             MYPCOL )
            ...
            ENDIF
            JCPOS = ( JC-1 ) + JSTARTB
            CALL PFCCOPYTAU( JSIZEB, 1, WORK( IP_TAUB ), DESCB, JCPOS,
     $                       TAUC, DESCC )
         ENDIF
*
*            Write out panel B.
*
         ...
         JCPOS = ( JC-1 ) + JSTARTB
         INFO = 0
         CALL CLAWRITE( IODEV, M, JSIZEB, ICPOS, JCPOS,
