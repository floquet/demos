      SUBROUTINE PZSHIFT( M, N, A, IA, JA, DESCA, B, IB, JB, DESCB,
     $                    INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   PCHK1MAT, PXERBLA, ZCOPY, ZGERV2D, ZGESD2D
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD
*     ..
...
*
     ...
     $                     MYPCOL )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PZSHIFT', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
     ...
     $               IDUMMY, INFO )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCB( CTXT_ ), 'PZSHIFT', 10 )
         RETURN
      ENDIF
      IDUMMY = 1
     ...
     $               IDUMMY, INFO )
      ...
      IF( MOD( IA-1, MB ).NE.MOD( IB-1, MB ) ) THEN
         INFO = -4
         CALL PXERBLA( DESCA( CTXT_ ), 'PZSHIFT', ( -INFO ) )
         RETURN
      ENDIF
...
*
         ...
         DO 10 J = 1, NNA
            CALL ZCOPY( MMA, A( ISRC ), 1, B( IDEST ), 1 )
...
*
         ...
         IF( ( MMA.GE.1 ) .AND. ( NNA.GE.1 ) ) THEN
            CALL ZGESD2D( DESCA( CTXT_ ), MMA, NNA, A( ISRC ),
     $                    DESCA( LLD_ ), IDEST_ROW, IDEST_COL )
         ENDIF
         IF( ( MMB.GE.1 ) .AND. ( NNB.GE.1 ) ) THEN
            CALL ZGERV2D( DESCB( CTXT_ ), MMB, NNB, B( IDEST ),
