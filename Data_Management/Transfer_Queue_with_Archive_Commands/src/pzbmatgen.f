      SUBROUTINE PZBMATGEN( ICTXT, AFORM, DIAG, BWL, BWU, N, MB, NB, A,
...
*
         CALL PZMATGEN( ICTXT, 'T', 'N', N_MATGEN, M_MATGEN, NB,
...
*
      ELSE
*
         CALL PZMATGEN( ICTXT, 'N', 'N', M_MATGEN, N_MATGEN, M_MATGEN,
...
*
         DO 10 I = 1, NQ
            IF( LSAME( AFORM, 'T' ) ) THEN
               IF( NPROW.EQ.1 ) THEN
                  A( I, DIAG_INDEX ) = DCMPLX( DBLE( A( I,
     $                                 DIAG_INDEX ) )+2*( BWL+BWU+1 ) )
               ENDIF
            ELSE
               IF( NPROW.EQ.1 ) THEN
                  A( DIAG_INDEX, I ) = DCMPLX( DBLE( A( DIAG_INDEX,
     $                                 I ) )+2*( BWL+BWU+1 ) )
               ENDIF
            ENDIF
   10    CONTINUE
