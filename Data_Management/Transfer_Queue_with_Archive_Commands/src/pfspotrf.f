      SUBROUTINE PFSPOTRF( UPLO, N, A, IA, JA, DESCA, INFO )
*
*
...
*
     ...
     $                     MYPCOL )
      ...
      IF( .NOT.ISVALID ) THEN
         INFO = -1
         CALL PXERBLA( DESCA( CTXT_ ), 'PFSPOTRF', 1 )
         RETURN
      ENDIF
      SAVEDT = DESCA( DT_ )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFSPOTRF', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
      ...
      ASIZE = DESCA( SIZE_ )
      IF( LSAME( UPLO, 'L' ) ) THEN
         CALL PFSLCHFACT( IODEV, N, IA, JA, A, ASIZE, INFO )
      ELSE
         CALL PFSUCHFACT( IODEV, N, IA, JA, A, ASIZE, INFO )
      ENDIF
      A( 1 ) = ANEED
      RETURN
      END
