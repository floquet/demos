      SUBROUTINE PCMATGEN( ICTXT, AFORM, DIAG, M, N, MB, NB, A, LDA,
...
*  =======
*
*  PCMATGEN : Parallel Complex Single precision MATrix GENerator.
...
*     .. Scalar Arguments ..
     ...
     $                   IROFF, ISEED, LDA, M, MB, MYCOL, MYROW, N, NB,
     $                   NPCOL, NPROW
*     ..
*     .. Array Arguments ..
      COMPLEX            A( LDA, * )
...
*     ..
*     .. Local Arrays ..
      INTEGER            IA1( 2 ), IA2( 2 ), IA3( 2 ), IA4( 2 ),
     $                   IA5( 2 ), IADD( 2 ), IB1( 2 ), IB2( 2 ),
...
*     .. External Functions ..
      ...
      REAL               PSRAND
      EXTERNAL           LSAME, ICEIL, NUMROC, PSRAND
*     ..
*     .. External Subroutines ..
      EXTERNAL           JUMPIT, PXERBLA, SETRAN, XJUMPM
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, AIMAG, CMPLX, CONJG, MAX, MOD, REAL
*     ..
...
*
      INFO = 0
      IF( .NOT.( SYMM .OR. HERM .OR. TRAN ) .AND. .NOT.
     $    LSAME( AFORM, 'C' ) .AND. .NOT.LSAME( AFORM, 'N' ) ) THEN
         INFO = 2
      ELSEIF( .NOT.LSAME( DIAG, 'D' ) .AND. .NOT.LSAME( DIAG, 'N' ) )
     $         THEN
         INFO = 3
      ELSEIF( SYMM .OR. HERM ) THEN
         IF( M.NE.N ) THEN
            INFO = 5
         ELSEIF( MB.NE.NB ) THEN
            INFO = 7
         ENDIF
      ELSEIF( M.LT.0 ) THEN
         INFO = 4
      ...
      ELSEIF( NB.LT.1 ) THEN
         INFO = 7
      ELSEIF( LDA.LT.0 ) THEN
         INFO = 9
      ELSEIF( ( IAROW.LT.0 ) .OR. ( IAROW.GE.NPROW ) ) THEN
         INFO = 10
      ELSEIF( ( IACOL.LT.0 ) .OR. ( IACOL.GE.NPCOL ) ) THEN
         INFO = 11
      ELSEIF( MOD( IROFF, MB ).GT.0 ) THEN
         INFO = 13
      ELSEIF( IRNUM.GT.( MP-IROFF ) ) THEN
         INFO = 14
      ELSEIF( MOD( ICOFF, NB ).GT.0 ) THEN
         INFO = 15
      ELSEIF( ICNUM.GT.( NQ-ICOFF ) ) THEN
         INFO = 16
      ELSEIF( ( MYROW.LT.0 ) .OR. ( MYROW.GE.NPROW ) ) THEN
         INFO = 17
      ELSEIF( ( MYCOL.LT.0 ) .OR. ( MYCOL.GE.NPCOL ) ) THEN
         INFO = 18
      ENDIF
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( ICTXT, 'PCMATGEN', INFO )
         RETURN
      ENDIF
*
...
*
   ...
   20                CONTINUE
*
                  ELSEIF( IOFFC.EQ.IOFFR ) THEN
                     IK = IK + I - 1
                     IF( IK.GT.IRNUM )
     $                  GOTO 60
                     DO 30 J = 1, I - 1
c                       A( IK, JK ) = CMPLX(PSRAND( 0 ), PSRAND( 0 ))
                        DUMMY=PSRAND(0)
                        DUMMY2=PSRAND(0)
   30                CONTINUE
                     A( IK, JK ) = CMPLX(DUMMY, DUMMY2)
                     IF( SYMM ) THEN
                        A( IK, JK ) = CMPLX( ONE-TWO*PSRAND( 0 ),
     $                                ONE-TWO*PSRAND( 0 ) )
                     ELSE
                        A( IK, JK ) = CMPLX(ONE-TWO*PSRAND( 0 ), ZERO)
                        DUMMY = PSRAND( 0 )
                     ENDIF
                     DO 40 J = 1, MB - I
                        IF( IK+J.GT.IRNUM )
     $                     GOTO 60
                        A( IK+J, JK ) = CMPLX( ONE-TWO*PSRAND( 0 ),
     $                                  ONE-TWO*PSRAND( 0 ) )
                        IF( HERM ) THEN
                           A( IK, JK+J ) = CONJG( A( IK+J, JK ) )
                        ELSE
                           A( IK, JK+J ) = A( IK+J, JK )
                        ENDIF
   40                CONTINUE
...
*
      ELSEIF( TRAN .OR. LSAME( AFORM, 'C' ) ) THEN
*
         JUMP1 = 1
         JUMP2 = 2*NQNB
         JUMP3 = 2*N
