      SUBROUTINE PFSGEQRS( M, N, NRHS, A, IA, JA, DESCA, TAU, X, IX, JX,
...
*     .. External Subroutines ..
     ...
     $                   LAIO_INFO, PCHK1MAT, PFMAXSIZE, PFSQRSOLVE,
     $                   PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*     .. Executable Statements ..
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFSGEQRS', 7 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
 ...
 9999    FORMAT( '** PFGEQRS: ', ' myprow,mypcol ', 2( 1X, I4 ),
     ...
     $               DESCA( CTXT_ ), DESCA( LLD_ ), INFO )
      CALL ASSERT( INFO.EQ.0, '** PFGEQRS: descinit returns info ',
     $             INFO )
      CALL PFSQRSOLVE( IODEV, M, N, NRHS, IA, JA, DESC2, TAU, A, DESC1,
     $                 ASIZE, X, IX, JX, DESCX, WORK, LWORK )
      ...
      IF( INEED.GT.LWORK ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICTXT, 'PFSGEQRS', 14 )
         ENDIF
         RETURN
      ENDIF
      INFO = 0
