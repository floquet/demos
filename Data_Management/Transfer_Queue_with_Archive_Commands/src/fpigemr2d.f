      SUBROUTINE FPIGEMR2D( M, N, AMAT, IA, JA, DESCA, BMAT, IB, JB,
     $                      DESCB )
*     .. Parameters ..
      INTEGER            DLEN_
      PARAMETER          ( DLEN_ = 9 )
...
*     ..
*     .. Array Arguments ..
      INTEGER            AMAT( * ), BMAT( * ), DESCA( DLEN_ ),
     $                   DESCB( DLEN_ )
*     ..
*     .. Local Scalars ..
      LOGICAL            ISALIGNED, ISOK, ISVALID, USEINLINE
      INTEGER            AIJ, CONTXTA, CONTXTB, CSRCA, CSRCB, I, ICOL,
     $                   IDXA, IDXB, II, INFO, IROW, ISIZEA, ISIZEB, J,
     $                   JJ, JSIZEA, JSIZEB, LCINDX, LDA, LDB, LRINDX,
     $                   MAXIA, MAXIB, MAXJA, MAXJB, MBA, MBB, MINIA,
     $                   MINIB, MINJA, MINJB, MYPCOLA, MYPCOLB, MYPROWA,
     $                   MYPROWB, NBA, NBB, NCOLA, NCOLB, NPCOLA,
     $                   NPCOLB, NPROWA, NPROWB, NROWA, NROWB, ONE,
     $                   RSRCA, RSRCB, ZERO
*     ..
*     .. External Functions ..
      LOGICAL            CHECKALIGN
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, ICOPY, INFOG2L,
     $                   PIGEMR2DO, PISHIFT
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          INT, MOD
   ...
   40          CONTINUE
            ELSE
               ...
               DO 50 J = 1, JSIZEA
                  ...
                  JJ = MINJB + ( J-1 )
                  IDXB = II + ( JJ-1 )*LDB
                  CALL ICOPY( ISIZEA, AMAT( IDXA ), 1, BMAT( IDXB ), 1 )
   50          CONTINUE
   60          CONTINUE
            ENDIF
         ENDIF
...
*
     ...
     $             MOD( JB-1, DESCB( NB_ ) ) )
         ...
         IF( ISOK ) THEN
            INFO = 0
            CALL PISHIFT( M, N, AMAT, IA, JA, DESCA, BMAT, IB, JB,
     $                    DESCB, INFO )
            CALL ASSERT( INFO.EQ.0,
     $                   '** FPxGEMR2D: PxSHIFT returns info= ', INFO )
         ELSE
            CALL PIGEMR2DO( M, N, AMAT, IA, JA, DESCA, BMAT, IB, JB,
     $                      DESCB )
         ENDIF
      ENDIF
      RETURN
