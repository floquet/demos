      SUBROUTINE PGCTRSM( SIDE, UPLO, TRANS, DIAG, M, N, ALPHA, A, IA,
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, BLACS_PINFO, CFILL,
     $                   CGERV2D, CGESD2D, CHK1MAT, DESCINIT, INFOG2L,
     $                   PCGEMM, PCHK1MAT, PCSCAL, PCTRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                     MYPCOL )
      ...
      IF( .NOT.ISVALID ) THEN
         INFO = -1
         CALL PXERBLA( ICONTXT, 'PGCTRSM', 1 )
         RETURN
      ENDIF
     ...
     $          LSAME( TRANS, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -3
         CALL PXERBLA( ICONTXT, 'PGCTRSM', 3 )
         RETURN
      ENDIF
      ISVALID = ( LSAME( UPLO, 'U' ) .OR. LSAME( UPLO, 'L' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -2
         CALL PXERBLA( ICONTXT, 'PGCTRSM', 2 )
         RETURN
      ENDIF
      ISVALID = ( LSAME( DIAG, 'U' ) .OR. LSAME( DIAG, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -4
         CALL PXERBLA( ICONTXT, 'PGCTRSM', 4 )
         RETURN
      ENDIF
*
*
...
*
      ...
      IF( ISLEFT ) THEN
         ...
         IF( INFO.NE.0 ) THEN
            CALL PXERBLA( DESCA( CTXT_ ), 'PGCTRSM', 11 )
            RETURN
         ENDIF
         IDUMMY = 1
     ...
     $                  IDUMMY, INFO )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCB( CTXT_ ), 'PGCTRSM', 15 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
     ...
     $                 ( ICOLA.EQ.ICOLB ) )
      IF( ( USE_PTRSM ) .AND. ( ISTRSM_ALIGNED ) ) THEN
         CALL PCTRSM( SIDE, UPLO, TRANS, DIAG, M, N, ALPHA, A, IA, JA,
     $                DESCA, B, IB, JB, DESCB )
         WORK( 1 ) = 0
         RETURN
*
...
*
            ...
            IF( HASWORK ) THEN
               IF( ISLEFT ) THEN
                  CALL PCGEMM( TRANSA, 'N', MM, NN, KK, LALPHA, A,
     $                         IAPOS, JAPOS, DESCA, B, IBPOS, JBPOS,
     $                         DESCB, LBETA, B, ICPOS, JCPOS, DESCB )
               ELSE
                  CALL PCGEMM( 'N', TRANSA, MM, NN, KK, LALPHA, B,
...
*
            LDATMP = JINC
            CALL CFILL( JINC*JINC, ZERO, WORK( IP_ATMP ), 1 )
*
...
* Package and send
     ...
     $                   '** PGTRSM: invalid lcoff = ', LCOFF )
            CALL PCTRSM( SIDE, UPLO, TRANS, DIAG, MM, NN, LALPHA,
...
*
            CALL PCTRSM( SIDE, UPLO, TRANS, DIAG, MM, NN, LALPHA, A,
...
* -----------------------------------
      IF( ALPHA.NE.ONE ) THEN
         DO 50 JSTART = 1, N
            ...
            JBPOS = ( JB-1 ) + JSTART
            INC = 1
            CALL PCSCAL( M, ALPHA, B, IBPOS, JBPOS, DESCB, INC )
   50    CONTINUE
   60    CONTINUE
      ENDIF
      WORK( 1 ) = INEED
