      SUBROUTINE PFCLCHFACT( IODEV, M, IC, JC, WORK, LWORK, INFO )
*
*
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, CLAREAD, CLAWRITE,
     $                   DESCINIT, LAIO_INFO, PCGEMM, PCHERK, PCPOTRF,
     $                   PCTRSM, PFMAXSIZE, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICONTXT, 'PFCLCHFACT', -INFO )
         ENDIF
         RETURN
      ENDIF
*
...
*
      ...
      IIC = ( IC-1 ) + ISTARTB
      JJC = ( JC-1 ) + JSTARTB
      CALL CLAREAD( IODEV, ISIZEB, JSIZEB, IIC, JJC, WORK( IP_BMAT ), 1,
     $              1, DESCB, INFO )
...
*
         LINFO = 0
         CALL PCPOTRF( 'Lower', JSIZEB, WORK( IP_BMAT ), 1, 1, DESCB,
     $                 LINFO )
         IF( LINFO.GE.1 ) THEN
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PCTRSM( 'Rightside', 'LowerTriangular', 'C',
...
*
            LINFO = 0
            CALL PCPOTRF( 'Lower', ISIZE, WORK( IP_BMAT ), ISTART,
     $                    ISTART, DESCB, LINFO )
            IF( LINFO.GE.1 ) THEN
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PCHERK( 'Lower', 'NoTranspose', NN, KK,
     $                      REAL( ALPHA ), WORK( IP_BMAT ), ( IEND+1 ),
     $                      ISTART, DESCB, REAL( BETA ),
...
*
            ...
            IF( HASWORK ) THEN
               CALL PCGEMM( 'NoTranspose', 'C', MM, NN, KK, ALPHA,
...
*
      IIC = ( IC-1 ) + ISTARTB
      JJC = ( JC-1 ) + JSTARTB
      CALL CLAWRITE( IODEV, ISIZEB, JSIZEB, IIC, JJC, WORK( IP_BMAT ),
     $               1, 1, DESCB, INFO )
...
*
         ...
         IIC = ( IC-1 ) + ISTARTB
         JJC = ( JC-1 ) + JSTARTB
         CALL CLAREAD( IODEV, ISIZEB, JSIZEB, IIC, JJC, WORK( IP_BMAT ),
     $                 1, 1, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '** PFxLCHFACT: LAREAD returns info = '
     $                , INFO )
         JSTARTA = 1
...
*
            ...
            IF( HASWORK ) THEN
               CALL PCHERK( 'Lower', 'NoTranspose', NN, KK,
     $                      REAL( ALPHA ), WORK( IP_AMAT ), 1, 1, DESCA,
     $                      REAL( BETA ), WORK( IP_BMAT ), 1, 1, DESCB )
            ENDIF
*
...
*
               ...
               IF( HASWORK ) THEN
                  CALL PCHERK( 'Lower', 'NoTranspose', NN, KK,
     $                         REAL( ALPHA ), WORK( IP_BMAT ),
     $                         ( IEND+1 ), ISTART, DESCB, REAL( BETA ),
     $                         WORK( IP_BMAT ), ( IEND+1 ), ( IEND+1 ),
     $                         DESCB )
               ENDIF
*
...
*
         ...
         IIC = ( IC-1 ) + ISTARTB
         JJC = ( JC-1 ) + JSTARTB
         CALL CLAWRITE( IODEV, ISIZEB, JSIZEB, IIC, JJC,
