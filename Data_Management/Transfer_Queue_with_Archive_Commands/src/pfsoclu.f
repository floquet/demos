      SUBROUTINE PFSOCLU( IODEV, NNB, M, N, IC, JC, DESCC, IPIV, LDPIV,
     $                    WORK, LWORK, INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   PSGEMM, PSGETRF, PSLAPIV, PSTRSM, SLAREAD,
     $                   SLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL SLAREAD( IODEV, M, GNCOLB, IC, JC, WORK( IP_BMAT ), 1, 1,
     $              DESCB, INFO )
      CALL ASSERT( INFO.EQ.0, '** PFOCLU: dlread returns info ', INFO )
      CALL PSGETRF( M, GNCOLB, WORK( IP_BMAT ), 1, 1, DESCB, IPIV,
     $              INFO )
...
*
         ...
         IF( HASWORK ) THEN
            CALL PSLAPIV( 'B', 'R', 'C', MM, NN, WORK( IP_BMAT ), 1, 1,
     $                    DESCB, IPIV, 1, 1, DESCIPIV, IDUM )
         ENDIF
      ENDIF
      CALL SLAWRITE( IODEV, M, GNCOLB, IC, JC, WORK( IP_BMAT ), 1, 1,
     $               DESCB, INFO )
...
*
         ICPOS = ( IC-1 ) + 1
         JCPOS = ( JC-1 ) + JSTARTB
         CALL SLAREAD( IODEV, M, JSIZEB, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                 1, 1, DESCB, INFO )
...
*
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL PSLAPIV( 'F', 'R', 'C', MM, NN, WORK( IP_BMAT ), 1, 1,
     $                    DESCB, IPIV, 1, 1, DESCIPIV, IDUM )
         ENDIF
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL SLAREAD( IODEV, M, JSIZEA, ICPOS, JCPOS,
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PSLAPIV( 'F', 'R', 'C', MM, NN, WORK( IP_AMAT ), 1,
...
*
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PSTRSM( 'Left', 'Lower', 'No transpose', 'Unit', MM,
...
*
            ...
            IF( HASWORK ) THEN
               CALL PSGEMM( 'No transpose', 'No transpose', MM, NN, KK,
...
* ========================================
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL PSGETRF( MM, NN, WORK( IP_BMAT ), IB, JB, DESCB, IPIV,
     $                    INFO )
...
* ---------------------------
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL SLAWRITE( IODEV, MM, NN, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                     1, 1, DESCB, INFO )
...
*
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL SLAREAD( IODEV, MM, NN, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                    1, 1, DESCB, INFO )
            CALL ASSERT( INFO.EQ.0, '** PFOCLU: laread returns info ',
     $                   INFO )
         ENDIF
