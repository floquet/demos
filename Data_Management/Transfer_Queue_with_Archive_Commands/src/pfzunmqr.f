      SUBROUTINE PFZUNMQR( SIDE, TRANS, M, N, K, A, IA, JA, DESCA, TAU,
...
*     .. External Subroutines ..
     ...
     $                   LAIO_INFO, PCHK1MAT, PFMAXSIZE, PFZCOPYTAU,
     $                   PXERBLA, PZUNMQR, ZLAREAD
*     ..
*     .. Intrinsic Functions ..
...
* error checking
     ...
     $                     MYPCOL )
      ...
      IF( .NOT.ISVALID ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFZORMQR', 1 )
         INFO = -1
         RETURN
      ENDIF
      ...
      IF( .NOT.ISVALID ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFZORMQR', 2 )
         INFO = -2
         RETURN
      ENDIF
      SAVEDT = DESCA( DT_ )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFZORMQR', 9 )
         RETURN
      ENDIF
      IDUMMY = 1
     ...
     $               IDUMMY, INFO )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCC( CTXT_ ), 'PFZORMQR', 14 )
         RETURN
      ENDIF
      IDUMMY = 1
     ...
     $             , INFO )
      ...
      IF( NFREE.LE.0 ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( DESCA( CTXT_ ), 'PFZORMQR', 16 )
         ENDIF
         RETURN
      ENDIF
      LINFO = 0
...
*
     ...
     $                      '** PFOMQR: descinit returns info ', INFO )
               CALL ZLAREAD( DESCA( IODEV_ ), ISIZE, JSIZE, ISTART,
     $                       JSTART, A, IROFF, ICOFF, DESCATMP, INFO )
               CALL PFZCOPYTAU( JSIZE, JSTART, TAU, DESCAIN, ICOFF,
     $                          WORK( IPTAU ), DESCATMP )
               CALL PZUNMQR( SIDE, TRANS, MM, NN, KK, A, IROFF, ICOFF,
