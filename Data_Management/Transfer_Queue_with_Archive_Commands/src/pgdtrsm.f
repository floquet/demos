      SUBROUTINE PGDTRSM( SIDE, UPLO, TRANS, DIAG, M, N, ALPHA, A, IA,
...
*     .. External Subroutines ..
     ...
     $                   DESCINIT, DFILL, DGERV2D, DGESD2D, INFOG2L,
     $                   PCHK1MAT, PDGEMM, PDSCAL, PDTRSM, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                     MYPCOL )
      ...
      IF( .NOT.ISVALID ) THEN
         INFO = -1
         CALL PXERBLA( ICONTXT, 'PGDTRSM', 1 )
         RETURN
      ENDIF
     ...
     $          LSAME( TRANS, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -3
         CALL PXERBLA( ICONTXT, 'PGDTRSM', 3 )
         RETURN
      ENDIF
      ISVALID = ( LSAME( UPLO, 'U' ) .OR. LSAME( UPLO, 'L' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -2
         CALL PXERBLA( ICONTXT, 'PGDTRSM', 2 )
         RETURN
      ENDIF
      ISVALID = ( LSAME( DIAG, 'U' ) .OR. LSAME( DIAG, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -4
         CALL PXERBLA( ICONTXT, 'PGDTRSM', 4 )
         RETURN
      ENDIF
*
*
...
*
      ...
      IF( ISLEFT ) THEN
         ...
         IF( INFO.NE.0 ) THEN
            CALL PXERBLA( DESCA( CTXT_ ), 'PGDTRSM', 11 )
            RETURN
         ENDIF
         IDUMMY = 1
     ...
     $                  IDUMMY, INFO )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCB( CTXT_ ), 'PGDTRSM', 15 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
     ...
     $                 ( ICOLA.EQ.ICOLB ) )
      IF( ( USE_PTRSM ) .AND. ( ISTRSM_ALIGNED ) ) THEN
         CALL PDTRSM( SIDE, UPLO, TRANS, DIAG, M, N, ALPHA, A, IA, JA,
     $                DESCA, B, IB, JB, DESCB )
         WORK( 1 ) = 0
         RETURN
*
...
*
            ...
            IF( HASWORK ) THEN
               IF( ISLEFT ) THEN
                  CALL PDGEMM( TRANSA, 'N', MM, NN, KK, LALPHA, A,
...
*
            LDATMP = JINC
            CALL DFILL( JINC*JINC, ZERO, WORK( IP_ATMP ), 1 )
*
...
*
            CALL PDTRSM( SIDE, UPLO, TRANS, DIAG, MM, NN, LALPHA, A,
