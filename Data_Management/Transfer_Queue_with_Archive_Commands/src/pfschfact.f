      SUBROUTINE PFSCHFACT( IODEV, NNB, M, IC, JC, DESCC, WORK, LWORK,
     $                      INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   DESCINIT, PSGEMM, PSPOTRF, PSSYRK, PSTRSM,
     $                   SLAREAD, SLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      JCPOS = ( JC-1 ) + JSTARTB
      INFO = 0
      CALL SLAREAD( IODEV, ISIZEB, JSIZEB, ICPOS, JCPOS,
...
*
      CALL PSSYRK( UPLO, TRANS, NN, KK, ALPHA, WORK( IP_AMAT ), IA, JA,
...
*
         ...
         IB = 1 + JSIZEB
         JB = 1
         CALL PSGEMM( 'N', 'T', MM, NN, KK, ALPHA, WORK( IP_AMAT ), IA,
...
*
         ...
         JB = 1
         LINFO = 0
         CALL PSPOTRF( UPLO, JSIZEB, WORK( IP_BMAT ), IB, JB, DESCB,
     $                 LINFO )
         IF( INFO.GE.1 ) THEN
*
...
*
         ...
         IF( HASWORK ) THEN
            ...
            IB = JSIZEB + 1
            JB = 1
            CALL PSTRSM( SIDE, UPLO, TRANS, DIAG, ISIZEB, JSIZEB, ALPHA,
...
*
         ...
         NN = JJSIZE
         LINFO = 0
         CALL PSPOTRF( UPLO, NN, WORK( IP_BMAT ), IB, JB, DESCB, LINFO )
         IF( INFO.GE.1 ) THEN
*
*                       Part of matrix is not positive definite.
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PSTRSM( SIDE, UPLO, TRANS, DIAG, NN, KK, ALPHA,
     $                   WORK( IP_BMAT ), IA, JA, DESCB,
     $                   WORK( IP_BMAT ), IB, JB, DESCB )
         ENDIF
*
...
*
         ...
         HASWORK = ( NN.GE.1 ) .AND. ( KK.GE.1 )
         IF( HASWORK ) THEN
            CALL PSSYRK( UPLO, TRANS, NN, KK, ALPHA, WORK( IP_BMAT ),
...
*
         ...
         HASWORK = ( ( MM.GE.1 ) .AND. ( NN.GE.1 ) .AND. ( KK.GE.1 ) )
         IF( HASWORK ) THEN
            CALL PSGEMM( 'N', 'T', MM, NN, KK, ALPHA, WORK( IP_BMAT ),
...
*
      ...
      JCPOS = ( JC-1 ) + JSTARTB
      INFO = 0
      CALL SLAWRITE( IODEV, ISIZEB, JSIZEB, ICPOS, JCPOS,
     $               WORK( IP_BMAT ), 1, 1, DESCB, INFO )
