...
*     .. Local Scalars ..
     ...
     $                   NRHS, P0, Q0, RSRC
      REAL               DZERO, MAXERR, MAXRESID, NORM2, NORMB, NORMX
...
*     .. Local Arrays ..
     ...
     $                   PVAL( LDPVAL ), QVAL( LDQVAL )
      REAL               BNORM( MAXNRHS ), XNORM( MAXNRHS )
      COMPLEX            MEM( TOTMEM )
...
*     .. External Subroutines ..
     ...
     $                   IGSUM2D, LACLOSE, PCMATADD, PCMATGEN, PFCGEMM,
     $                   PFCGEQRF, PFCGEQRS, PFCGETRF, PFCGETRS,
     $                   PFCMATGEN, PFCPOTRF, PFCPOTRS, PFDESCINIT,
     $                   PSCNRM2, SGAMX2D, SLBOOT, SLCOMBINE, SLTIMER
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, CHAR, CMPLX, DBLE, INT, MAX, MIN, REAL
*     ..
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCA( SIZE_ ) = -1
                     MEM( IP_A ) = 0
                     CALL PFCMATGEN( DESCA( IODEV_ ), DESCA( CTXT_ ),
     $                               AFORM, DIAG, DESCA( M_ ),
     $                               DESCA( N_ ), DESCA( MB_ ),
     $                               DESCA( NB_ ), MEM( IP_A ),
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCAOLD( SIZE_ ) = -1
                     MEM( IP_AOLD ) = 0
                     CALL PFCMATGEN( DESCAOLD( IODEV_ ),
...
*
     ...
     $                    DESCX( CSRC_ ), NPCOL )
                  ISEED = 999999
                  CALL PCMATGEN( DESCX( CTXT_ ), AFORM, DIAG,
...
*
                  ...
                  DO 10 JX = 1, NRHS
                     NORM2 = -9999
                     CALL PSCNRM2( MIN( M, N ), NORM2, MEM( IP_X ), IX,
     $                             JX, DESCX, INC )
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCAOLD( SIZE_ ) = -1
                     MEM( IP_A ) = 0
                     CALL PFCGEMM( 'NotransA', 'NotransX', M, NRHS, N,
...
*
                  DO 30 JB = 1, NRHS
                     NORM2 = -9999
                     CALL PSCNRM2( MIN( M, N ), NORM2, MEM( IP_B ), IB,
     $                             JB, DESCB, INC )
                     CALL SGAMX2D( ICTXT, 'A', ' ', 1, 1, NORM2, 1, 0,
     $                             0, -1, -1, -1 )
                     BNORM( JB ) = NORM2
...
*
                  ...
                  IR = 1
                  JR = 1
                  CALL PCMATADD( M, NRHS, ALPHA, MEM( IP_B ), IB, JB,
...
*
                        CALL PFCGEQRF( M, N, MEM( IP_A ), IA, JA, DESCA,
...
*
                        ...
                        MEM( IP_A ) = 0
                        MEM( IP_WORK ) = 0
                        CALL PFCGEQRS( M, N, NRHS, MEM( IP_A ), IA, JA,
     $                                 DESCA, MEM( IP_TAU ),
...
*
     ...
     $                           'PFxPOTRF returns info ', IERR( 1 )
                        ...
                        IF( ASIZEQUERY ) THEN
                           ...
                           DESCA( SIZE_ ) = -1
                           MEM( IP_A ) = 0
                           CALL PFCPOTRS( UPLO, MIN( M, N ), NRHS,
     $                                    MEM( IP_A ), IA, JA, DESCA,
     $                                    MEM( IP_R ), IR, JR, DESCR,
     $                                    INFO )
                           ANEED = INT( DBLE( 100 )*DEPS+
...
*
                           ...
                           IF( ASIZEQUERY ) THEN
                              ...
                              DESCA( SIZE_ ) = -1
                              MEM( IP_A ) = 0
                              CALL PFCGETRF( M, N, MEM( IP_A ), IA, JA,
     $                                       DESCA, IMEM( IP_IPIV ),
     $                                       INFO )
                              ANEED = INT( DBLE( 100 )*DEPS+
     $                                ABS( MEM( IP_A ) ) )
...
*
                  ALPHA = -ONE
                  BETA = ONE
                  CALL PCMATADD( MIN( M, N ), NRHS, ALPHA, MEM( IP_R ),
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCAOLD( SIZE_ ) = -1
                     MEM( IP_A ) = 0
                     CALL PFCGEMM( 'NotransA', 'NotransR', M, NRHS, N,
     $                             ALPHA, MEM( IP_A ), IA, JA, DESCAOLD,
