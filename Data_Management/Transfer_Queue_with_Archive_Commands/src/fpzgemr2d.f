      SUBROUTINE FPZGEMR2D( M, N, AMAT, IA, JA, DESCA, BMAT, IB, JB,
     $                      DESCB )
*     .. Parameters ..
      INTEGER            DLEN_
      PARAMETER          ( DLEN_ = 9 )
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, INFOG2L, PZGEMR2DO,
     $                   PZSHIFT, ZCOPY
*     ..
*     .. Intrinsic Functions ..
   ...
   40          CONTINUE
            ELSE
               ...
               DO 50 J = 1, JSIZEA
                  ...
                  JJ = MINJB + ( J-1 )
                  IDXB = II + ( JJ-1 )*LDB
                  CALL ZCOPY( ISIZEA, AMAT( IDXA ), 1, BMAT( IDXB ), 1 )
   50          CONTINUE
   60          CONTINUE
            ENDIF
         ENDIF
...
*
     ...
     $             MOD( JB-1, DESCB( NB_ ) ) )
         ...
         IF( ISOK ) THEN
            INFO = 0
            CALL PZSHIFT( M, N, AMAT, IA, JA, DESCA, BMAT, IB, JB,
     $                    DESCB, INFO )
            CALL ASSERT( INFO.EQ.0,
     $                   '** FPxGEMR2D: PxSHIFT returns info= ', INFO )
         ELSE
            CALL PZGEMR2DO( M, N, AMAT, IA, JA, DESCA, BMAT, IB, JB,
     $                      DESCB )
         ENDIF
      ENDIF
      RETURN
