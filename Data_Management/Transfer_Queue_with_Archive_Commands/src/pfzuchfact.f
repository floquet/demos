      SUBROUTINE PFZUCHFACT( IODEV, M, IC, JC, WORK, LWORK, INFO )
*
*
...
*
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICONTXT, 'PFZUCHFACT', -INFO )
         ENDIF
         RETURN
      ENDIF
*
...
*
         LINFO = 0
         CALL PZPOTRF( 'Upper', ISIZEB, WORK( IP_BMAT ), 1, 1, DESCB,
     $                 LINFO )
         IF( LINFO.GE.1 ) THEN
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PZTRSM( 'Leftside', 'UpperTriangular', 'C',
...
*
            LINFO = 0
            CALL PZPOTRF( 'Upper', ISIZE, WORK( IP_BMAT ), ISTART,
     $                    ISTART, DESCB, LINFO )
            IF( LINFO.GE.1 ) THEN
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PZHERK( 'Upper', 'C', NN, KK, DBLE( ALPHA ),
     $                      WORK( IP_BMAT ), ISTART, ( IEND+1 ), DESCB,
     $                      DBLE( BETA ), WORK( IP_BMAT ), ( IEND+1 ),
     $                      ( IEND+1 ), DESCB )
            ENDIF
*
* Update the rest
...
*
            ...
            IF( HASWORK ) THEN
               CALL PZGEMM( 'C', 'NoTranspose', MM, NN, KK, ALPHA,
     $                      WORK( IP_BMAT ), ISTART, ( IEND+1 ), DESCB,
...
*
            ...
            IIC = ( IC-1 ) + ISTARTA
            JJC = ( JC-1 ) + JSTARTB
            CALL ZLAREAD( IODEV, ISIZEA, JSIZEB, IIC, JJC,
...
*
            ...
            IF( HASWORK ) THEN
               CALL PZHERK( 'Upper', 'C', NN, KK, DBLE( ALPHA ),
     $                      WORK( IP_AMAT ), 1, 1, DESCA, DBLE( BETA ),
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PZTRSM( 'Leftside', 'UpperPart', 'C',
     $                      'NonunitDiagonal', MM, NN, ALPHA,
     $                      WORK( IP_BMAT ), 1, 1, DESCB,
