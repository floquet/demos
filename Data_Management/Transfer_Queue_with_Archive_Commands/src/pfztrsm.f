      SUBROUTINE PFZTRSM( SIDE, UPLO, TRANS, DIAG, M, N, ALPHA, A, IA,
...
* ===========================================================
     ...
     $                     MYPCOL )
      ...
      IF( .NOT.ISVALID ) THEN
         INFO = -1
         CALL PXERBLA( ICONTXT, 'PFZTRSM', 1 )
         RETURN
      ENDIF
     ...
     $          LSAME( TRANS, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -3
         CALL PXERBLA( ICONTXT, 'PFZTRSM', 3 )
         RETURN
      ENDIF
      ISVALID = ( LSAME( UPLO, 'U' ) .OR. LSAME( UPLO, 'L' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -2
         CALL PXERBLA( ICONTXT, 'PFZTRSM', 2 )
         RETURN
      ENDIF
      ISVALID = ( LSAME( DIAG, 'U' ) .OR. LSAME( DIAG, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -4
         CALL PXERBLA( ICONTXT, 'PFZTRSM', 4 )
         RETURN
      ENDIF
      WORK( 1 ) = 0
*
...
*
      ...
      IF( ISLEFT ) THEN
         ...
         IF( INFO.NE.0 ) THEN
            CALL PXERBLA( DESCA( CTXT_ ), 'PFZTRSM', 11 )
            RETURN
         ENDIF
         IDUMMY = 1
     ...
     $                  IDUMMY, INFO )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCB( CTXT_ ), 'PFZTRSM', 15 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
      ...
      IF( ANEED.GT.ASIZE ) THEN
         ...
         RETURN
      ENDIF
      CALL PFZLATRSM( SIDE, UPLO, TRANS, DIAG, DESCA( IODEV_ ), M, N,
