      SUBROUTINE PFCPOTRF( UPLO, N, A, IA, JA, DESCA, INFO )
*
*
...
*
     ...
     $                     MYPCOL )
      ...
      IF( .NOT.ISVALID ) THEN
         INFO = -1
         CALL PXERBLA( DESCA( CTXT_ ), 'PFCPOTRF', 1 )
         RETURN
      ENDIF
      SAVEDT = DESCA( DT_ )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFCPOTRF', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
      ...
      ASIZE = DESCA( SIZE_ )
      IF( LSAME( UPLO, 'L' ) ) THEN
         CALL PFCLCHFACT( IODEV, N, IA, JA, A, ASIZE, INFO )
      ELSE
         CALL PFCUCHFACT( IODEV, N, IA, JA, A, ASIZE, INFO )
      ENDIF
      A( 1 ) = ANEED
      RETURN
      END
