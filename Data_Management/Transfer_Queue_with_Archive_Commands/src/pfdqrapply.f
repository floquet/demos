      SUBROUTINE PFDQRAPPLY( IODEV, M, N, K, IC, JC, DESCC, TAUC, A,
...
*     .. Array Arguments ..
      ...
      DOUBLE PRECISION   A( * ), B( * ), TAUA( TAUA_DIM ), TAUC( * ),
     $                   WORK( LWORK )
*     ..
*     .. Local Scalars ..
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_PINFO, DFILL, DLAREAD, PDORMQR,
     $                   PFDCOPYTAU
*     ..
*     .. Intrinsic Functions ..
...
* not sure whether it should be ia = 1
         ...
         JCPOS = ( JC-1 ) + JSTARTA
         INFO = 0
         CALL DLAREAD( IODEV, ISIZEA, JSIZEA, ICPOS, JCPOS, A, IA, JA,
     $                 DESCA, INFO )
...
* copy tau
     ...
     $                '** qrapply: increase gtau_dim to ', JSIZEA+1 )
         CALL DFILL( TAUA_DIM, DZERO, TAUA( 1 ), 1 )
         JA = 1
         CALL PFDCOPYTAU( JSIZEA, JCPOS, TAUC, DESCC, JA, TAUA( 1 ),
     $                    DESCA )
*
...
*
     ...
     $             ( JSIZEA.GE.1 )
         IF( HASWORK ) THEN
            ...
            SIDE = 'L'
            TRANS = 'T'
            CALL PDORMQR( SIDE, TRANS, MM, NN, KK, A, IA, JA, DESCA,
