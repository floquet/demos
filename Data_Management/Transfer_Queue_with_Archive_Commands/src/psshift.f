      SUBROUTINE PSSHIFT( M, N, A, IA, JA, DESCA, B, IB, JB, DESCB,
     $                    INFO )
*
*
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, CHK1MAT, INFOG2L,
     $                   PCHK1MAT, PXERBLA, SCOPY, SGERV2D, SGESD2D
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MOD
*     ..
...
*
     ...
     $                     MYPCOL )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PSSHIFT', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
     ...
     $               IDUMMY, INFO )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCB( CTXT_ ), 'PSSHIFT', 10 )
         RETURN
      ENDIF
      IDUMMY = 1
     ...
     $               IDUMMY, INFO )
      ...
      IF( MOD( IA-1, MB ).NE.MOD( IB-1, MB ) ) THEN
         INFO = -4
         CALL PXERBLA( DESCA( CTXT_ ), 'PSSHIFT', ( -INFO ) )
         RETURN
      ENDIF
...
*
         ...
         DO 10 J = 1, NNA
            CALL SCOPY( MMA, A( ISRC ), 1, B( IDEST ), 1 )
...
*
         ...
         IF( ( MMA.GE.1 ) .AND. ( NNA.GE.1 ) ) THEN
            CALL SGESD2D( DESCA( CTXT_ ), MMA, NNA, A( ISRC ),
     $                    DESCA( LLD_ ), IDEST_ROW, IDEST_COL )
         ENDIF
         IF( ( MMB.GE.1 ) .AND. ( NNB.GE.1 ) ) THEN
            CALL SGERV2D( DESCB( CTXT_ ), MMB, NNB, B( IDEST ),
