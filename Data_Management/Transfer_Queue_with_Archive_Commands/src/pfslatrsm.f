      SUBROUTINE PFSLATRSM( SIDE, UPLO, TRANS, DIAG, IODEV, M, N, IC,
...
*     .. External Subroutines ..
     ...
     $                   LAIO_INFO, PFMAXSIZE, PSGEMM, PSSCAL, PSTRSM,
     $                   PXERBLA, SLAREAD
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( DESCB( CTXT_ ), 'PFSLATRSM', 16 )
         ENDIF
         RETURN
      ENDIF
...
* 1:jend
     ...
     $                  LDA, INFO )
         ...
         MM = ISIZE
         NN = JSIZE
         CALL SLAREAD( IODEV, ISIZE, JSIZE, ICPOS, JCPOS,
...
*
            ...
            IF( HASWORK ) THEN
               IF( ISLEFT ) THEN
                  CALL PSGEMM( TRANSA, 'N', MM, NN, KK, LALPHA,
