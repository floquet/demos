      SUBROUTINE PFSMATGEN( IODEV, ICTXT, AFORM, DIAG, M, N, MB, NB, A,
...
*     .. External Subroutines ..
     ...
     $                   INFOG2L, LAIO_INFO, PFMAX2SIZE, PSMATGEN,
     $                   PXERBLA, SLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( MB.NE.NB ) THEN
            INFO = -7
            CALL PXERBLA( ICTXT, 'PFSMATGEN', 7 )
            RETURN
         ENDIF
         MM = ILCM( MMB, NNB )
         NN = MM
...
*
   ...
   40    CONTINUE
     ...
     $                 ( MAX( II, JJ )-1 )*MAX( JJSIZE, IISIZE )
            ...
            IF( HASWORK ) THEN
               CALL PSMATGEN( ICTXT, LAFORM, LDIAG, ISIZE, JSIZE, MB,
...
*
 ...
 9999                      FORMAT( 'PFMATGEN: ', 'myprow,mypcol ',
   ...
   60             CONTINUE
               ENDIF
               CALL SLAWRITE( IODEV, ISIZE, JSIZE, ISTART, JSTART, A, 1,
     $                        1, DESCA, INFO )
