      SUBROUTINE PFCCHFACT( IODEV, NNB, M, IC, JC, DESCC, WORK, LWORK,
     $                      INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   CLAREAD, CLAWRITE, DESCINIT, PCGEMM, PCHERK,
     $                   PCPOTRF, PCTRSM
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      JCPOS = ( JC-1 ) + JSTARTB
      INFO = 0
      CALL CLAREAD( IODEV, ISIZEB, JSIZEB, ICPOS, JCPOS,
...
*
      CALL PCHERK( UPLO, TRANS, NN, KK, REAL( ALPHA ), WORK( IP_AMAT ),
     $             IA, JA, DESCA, REAL( BETA ), WORK( IP_BMAT ), IB, JB,
     $             DESCB )
      MM = ISIZEB - JSIZEB
      NN = JSIZEB
      KK = JSIZEA
...
*
         ...
         IB = 1 + JSIZEB
         JB = 1
         CALL PCGEMM( 'N', 'C', MM, NN, KK, ALPHA, WORK( IP_AMAT ), IA,
...
*
         ...
         JB = 1
         LINFO = 0
         CALL PCPOTRF( UPLO, JSIZEB, WORK( IP_BMAT ), IB, JB, DESCB,
     $                 LINFO )
         IF( INFO.GE.1 ) THEN
*
...
*
         ...
         IF( HASWORK ) THEN
            ...
            IB = JSIZEB + 1
            JB = 1
            CALL PCTRSM( SIDE, UPLO, TRANS, DIAG, ISIZEB, JSIZEB, ALPHA,
...
*
         ...
         NN = JJSIZE
         LINFO = 0
         CALL PCPOTRF( UPLO, NN, WORK( IP_BMAT ), IB, JB, DESCB, LINFO )
         IF( INFO.GE.1 ) THEN
*
*                       Part of matrix is not positive definite.
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PCTRSM( SIDE, UPLO, TRANS, DIAG, NN, KK, ALPHA,
     $                   WORK( IP_BMAT ), IA, JA, DESCB,
     $                   WORK( IP_BMAT ), IB, JB, DESCB )
         ENDIF
*
...
*
         ...
         HASWORK = ( NN.GE.1 ) .AND. ( KK.GE.1 )
         IF( HASWORK ) THEN
            CALL PCHERK( UPLO, TRANS, NN, KK, REAL( ALPHA ),
     $                   WORK( IP_BMAT ), IA, JA, DESCB, REAL( BETA ),
     $                   WORK( IP_BMAT ), IB, JB, DESCB )
         ENDIF
*
...
*
         ...
         HASWORK = ( ( MM.GE.1 ) .AND. ( NN.GE.1 ) .AND. ( KK.GE.1 ) )
         IF( HASWORK ) THEN
            CALL PCGEMM( 'N', 'C', MM, NN, KK, ALPHA, WORK( IP_BMAT ),
...
*
      ...
      JCPOS = ( JC-1 ) + JSTARTB
      INFO = 0
      CALL CLAWRITE( IODEV, ISIZEB, JSIZEB, ICPOS, JCPOS,
     $               WORK( IP_BMAT ), 1, 1, DESCB, INFO )
