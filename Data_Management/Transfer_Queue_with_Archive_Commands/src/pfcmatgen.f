      SUBROUTINE PFCMATGEN( IODEV, ICTXT, AFORM, DIAG, M, N, MB, NB, A,
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, CLAWRITE, DESCINIT,
     $                   IGSUM2D, INFOG2L, LAIO_INFO, PCMATGEN,
     $                   PFMAX2SIZE, PXERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, CMPLX, INT, MAX, MIN, MOD, REAL
*     ..
...
*
         IF( MB.NE.NB ) THEN
            INFO = -7
            CALL PXERBLA( ICTXT, 'PFCMATGEN', 7 )
            RETURN
         ENDIF
         MM = ILCM( MMB, NNB )
         NN = MM
...
*
   ...
   40    CONTINUE
     ...
     $                 ( MAX( II, JJ )-1 )*MAX( JJSIZE, IISIZE )
            ...
            IF( HASWORK ) THEN
               CALL PCMATGEN( ICTXT, LAFORM, LDIAG, ISIZE, JSIZE, MB,
...
*
 ...
 9999                      FORMAT( 'PFMATGEN: ', 'myprow,mypcol ',
   ...
   60             CONTINUE
               ENDIF
               CALL CLAWRITE( IODEV, ISIZE, JSIZE, ISTART, JSTART, A, 1,
     $                        1, DESCA, INFO )
