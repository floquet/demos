...
*     .. Local Scalars ..
     ...
     $                   NRHS, P0, Q0, RSRC
      DOUBLE PRECISION   ALPHA, BETA, DEPS, DZERO, MAXERR, MAXRESID,
     $                   NORM2, NORMB, NORMX, ONE, ZERO
*     ..
*     .. Local Arrays ..
     ...
     $                   PVAL( LDPVAL ), QVAL( LDQVAL )
      DOUBLE PRECISION   BNORM( MAXNRHS ), CTIME( 10 ), MEM( TOTMEM ),
     $                   WTIME( 10 ), XNORM( MAXNRHS )
*     ..
*     .. External Functions ..
      LOGICAL            LSAMEN
      INTEGER            NUMROC
...
*     .. External Subroutines ..
     ...
     $                   IGAMN2D, IGSUM2D, LACLOSE, PDMATADD, PDMATGEN,
     $                   PDNRM2, PFDESCINIT, PFDGEMM, PFDGEQRF,
     $                   PFDGEQRS, PFDGETRF, PFDGETRS, PFDMATGEN,
     $                   PFDPOTRF, PFDPOTRS, SLBOOT, SLCOMBINE, SLTIMER
*     ..
*     .. Intrinsic Functions ..
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCA( SIZE_ ) = -1
                     MEM( IP_A ) = 0
                     CALL PFDMATGEN( DESCA( IODEV_ ), DESCA( CTXT_ ),
     $                               AFORM, DIAG, DESCA( M_ ),
     $                               DESCA( N_ ), DESCA( MB_ ),
     $                               DESCA( NB_ ), MEM( IP_A ),
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCAOLD( SIZE_ ) = -1
                     MEM( IP_AOLD ) = 0
                     CALL PFDMATGEN( DESCAOLD( IODEV_ ),
...
*
     ...
     $                    DESCX( CSRC_ ), NPCOL )
                  ISEED = 999999
                  CALL PDMATGEN( DESCX( CTXT_ ), AFORM, DIAG,
...
*
                  ...
                  DO 10 JX = 1, NRHS
                     NORM2 = -9999
                     CALL PDNRM2( MIN( M, N ), NORM2, MEM( IP_X ), IX,
     $                            JX, DESCX, INC )
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCAOLD( SIZE_ ) = -1
                     MEM( IP_A ) = 0
                     CALL PFDGEMM( 'NotransA', 'NotransX', M, NRHS, N,
...
*
                  DO 30 JB = 1, NRHS
                     NORM2 = -9999
                     CALL PDNRM2( MIN( M, N ), NORM2, MEM( IP_B ), IB,
     $                            JB, DESCB, INC )
                     CALL DGAMX2D( ICTXT, 'A', ' ', 1, 1, NORM2, 1, 0,
     $                             0, -1, -1, -1 )
                     BNORM( JB ) = NORM2
...
*
                  ...
                  IR = 1
                  JR = 1
                  CALL PDMATADD( M, NRHS, ALPHA, MEM( IP_B ), IB, JB,
...
*
                        CALL PFDGEQRF( M, N, MEM( IP_A ), IA, JA, DESCA,
...
*
                        ...
                        MEM( IP_A ) = 0
                        MEM( IP_WORK ) = 0
                        CALL PFDGEQRS( M, N, NRHS, MEM( IP_A ), IA, JA,
     $                                 DESCA, MEM( IP_TAU ),
...
*
     ...
     $                           'PFxPOTRF returns info ', IERR( 1 )
                        ...
                        IF( ASIZEQUERY ) THEN
                           ...
                           DESCA( SIZE_ ) = -1
                           MEM( IP_A ) = 0
                           CALL PFDPOTRS( UPLO, MIN( M, N ), NRHS,
     $                                    MEM( IP_A ), IA, JA, DESCA,
     $                                    MEM( IP_R ), IR, JR, DESCR,
     $                                    INFO )
                           ANEED = INT( DBLE( 100 )*DEPS+
...
*
                           ...
                           IF( ASIZEQUERY ) THEN
                              ...
                              DESCA( SIZE_ ) = -1
                              MEM( IP_A ) = 0
                              CALL PFDGETRF( M, N, MEM( IP_A ), IA, JA,
     $                                       DESCA, IMEM( IP_IPIV ),
     $                                       INFO )
                              ANEED = INT( DBLE( 100 )*DEPS+
     $                                ABS( MEM( IP_A ) ) )
...
*
                  ALPHA = -ONE
                  BETA = ONE
                  CALL PDMATADD( MIN( M, N ), NRHS, ALPHA, MEM( IP_R ),
...
*
                  ...
                  IF( ASIZEQUERY ) THEN
                     ...
                     DESCAOLD( SIZE_ ) = -1
                     MEM( IP_A ) = 0
                     CALL PFDGEMM( 'NotransA', 'NotransR', M, NRHS, N,
     $                             ALPHA, MEM( IP_A ), IA, JA, DESCAOLD,
