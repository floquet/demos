      SUBROUTINE PFDOCLU( IODEV, NNB, M, N, IC, JC, DESCC, IPIV, LDPIV,
     $                    WORK, LWORK, INFO )
*
*
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, CALCOLSIZE, DESCINIT,
     $                   DLAREAD, DLAWRITE, PDGEMM, PDGETRF, PDLAPIV,
     $                   PDTRSM
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL DLAREAD( IODEV, M, GNCOLB, IC, JC, WORK( IP_BMAT ), 1, 1,
     $              DESCB, INFO )
      CALL ASSERT( INFO.EQ.0, '** PFOCLU: dlread returns info ', INFO )
      CALL PDGETRF( M, GNCOLB, WORK( IP_BMAT ), 1, 1, DESCB, IPIV,
     $              INFO )
...
*
         ...
         IF( HASWORK ) THEN
            CALL PDLAPIV( 'B', 'R', 'C', MM, NN, WORK( IP_BMAT ), 1, 1,
     $                    DESCB, IPIV, 1, 1, DESCIPIV, IDUM )
         ENDIF
      ENDIF
      CALL DLAWRITE( IODEV, M, GNCOLB, IC, JC, WORK( IP_BMAT ), 1, 1,
     $               DESCB, INFO )
...
*
         ICPOS = ( IC-1 ) + 1
         JCPOS = ( JC-1 ) + JSTARTB
         CALL DLAREAD( IODEV, M, JSIZEB, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                 1, 1, DESCB, INFO )
...
*
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL PDLAPIV( 'F', 'R', 'C', MM, NN, WORK( IP_BMAT ), 1, 1,
     $                    DESCB, IPIV, 1, 1, DESCIPIV, IDUM )
         ENDIF
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL DLAREAD( IODEV, M, JSIZEA, ICPOS, JCPOS,
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PDLAPIV( 'F', 'R', 'C', MM, NN, WORK( IP_AMAT ), 1,
...
*
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PDTRSM( 'Left', 'Lower', 'No transpose', 'Unit', MM,
...
*
            ...
            IF( HASWORK ) THEN
               CALL PDGEMM( 'No transpose', 'No transpose', MM, NN, KK,
...
* ========================================
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL PDGETRF( MM, NN, WORK( IP_BMAT ), IB, JB, DESCB, IPIV,
     $                    INFO )
...
* ---------------------------
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL DLAWRITE( IODEV, MM, NN, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                     1, 1, DESCB, INFO )
...
*
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL DLAREAD( IODEV, MM, NN, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                    1, 1, DESCB, INFO )
            CALL ASSERT( INFO.EQ.0, '** PFOCLU: laread returns info ',
     $                   INFO )
         ENDIF
