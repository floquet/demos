      SUBROUTINE PFCQRAPPLY( IODEV, M, N, K, IC, JC, DESCC, TAUC, A,
...
*     .. Array Arguments ..
      ...
      COMPLEX            A( * ), B( * ), TAUA( TAUA_DIM ), TAUC( * ),
     $                   WORK( LWORK )
*     ..
*     .. Local Scalars ..
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_PINFO, CFILL, CLAREAD, PCUNMQR,
     $                   PFCCOPYTAU
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, CMPLX, MAX, MIN, MOD, REAL
*     ..
...
* not sure whether it should be ia = 1
         ...
         JCPOS = ( JC-1 ) + JSTARTA
         INFO = 0
         CALL CLAREAD( IODEV, ISIZEA, JSIZEA, ICPOS, JCPOS, A, IA, JA,
     $                 DESCA, INFO )
...
* copy tau
     ...
     $                '** qrapply: increase gtau_dim to ', JSIZEA+1 )
         CALL CFILL( TAUA_DIM, DZERO, TAUA( 1 ), 1 )
         JA = 1
         CALL PFCCOPYTAU( JSIZEA, JCPOS, TAUC, DESCC, JA, TAUA( 1 ),
     $                    DESCA )
*
...
*
     ...
     $             ( JSIZEA.GE.1 )
         IF( HASWORK ) THEN
            ...
            SIDE = 'L'
            TRANS = 'C'
            CALL PCUNMQR( SIDE, TRANS, MM, NN, KK, A, IA, JA, DESCA,
