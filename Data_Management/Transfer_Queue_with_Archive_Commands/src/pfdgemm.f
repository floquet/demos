      SUBROUTINE PFDGEMM( TRANSA, TRANSB, M, N, K, ALPHA, A, IA, JA,
...
*     .. External Subroutines ..
     ...
     $                   DLAREAD, DLAWRITE, IGSUM2D, LAIO_INFO,
     $                   PCHK1MAT, PDGEMM, PDSCAL, PFMAX2SIZE, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $          LSAME( TRANSA, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -1
         CALL PXERBLA( DESCA( CTXT_ ), 'PFDGEMM', 1 )
         RETURN
      ENDIF
     ...
     $          LSAME( TRANSB, 'N' ) )
      IF( .NOT.ISVALID ) THEN
         INFO = -2
         CALL PXERBLA( DESCB( CTXT_ ), 'PFDGEMM', 2 )
         RETURN
      ENDIF
...
*
         ...
         IF( INFO.NE.0 ) THEN
            CALL PXERBLA( DESCA( CTXT_ ), 'PFDGEMM', 10 )
            RETURN
         ENDIF
         IDUMMY = 1
...
*
         ...
         IF( INFO.NE.0 ) THEN
            CALL PXERBLA( DESCB( CTXT_ ), 'PFDGEMM', 14 )
            RETURN
         ENDIF
         IDUMMY = 1
...
*
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCC( CTXT_ ), 'PFDGEMM', 16 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
         CALL PDGEMM( TRANSA, TRANSB, M, N, K, ALPHA, A, IA, JA, DESCA,
     $                B, IB, JB, DESCB, BETA, C, IC, JC, DESCC )
      ...
      IF( NDISK.NE.1 ) THEN
         ...
         IF( BONDISK .AND. CONDISK ) THEN
            INFO = -16
         ENDIF
         CALL PXERBLA( DESCA( CTXT_ ), 'PFDGEMM', -INFO )
         RETURN
      ENDIF
      IF( CONDISK ) THEN
         CSIZE = DESCC( SIZE_ )
...
*
     ...
     $                   '** PFxGEMM: descinit returns info = ', INFO )
            INFO = 0
            CALL DLAREAD( IODEV, M, N, IC, JC, C, 1, 1, DESCCTMP, INFO )
            CALL ASSERT( INFO.EQ.0, '** PFxGEMM: LAREAD returns info = '
     $                   , INFO )
            CALL PDGEMM( TRANSA, TRANSB, M, N, K, ALPHA, A, IA, JA,
     $                   DESCA, B, IB, JB, DESCB, BETA, C, 1, 1,
     $                   DESCCTMP )
            INFO = 0
            CALL DLAWRITE( IODEV, M, N, IC, JC, C, 1, 1, DESCCTMP,
     $                     INFO )
            CALL ASSERT( INFO.EQ.0,
...
*
     ...
     $                         INFO )
                  CALL DLAREAD( IODEV, ISIZE, JSIZE, IIC, JJC, C, IOFF,
     $                          JOFF, DESCCTMP, INFO )
                  CALL ASSERT( INFO.EQ.0,
     $                         '** PFxGEMM: LAREAD returns info ',
     $                         INFO )
                  CALL PDGEMM( TRANSA, TRANSB, MM, NN, KK, ALPHA, A,
     $                         IIA, JJA, DESCA, B, IIB, JJB, DESCB,
     $                         BETA, C, IOFF, JOFF, DESCC )
                  CALL DLAWRITE( IODEV, ISIZE, JSIZE, IIC, JJC, C, IOFF,
     $                           JOFF, DESCCTMP, INFO )
               ENDIF
* end if (haswork)
   10       CONTINUE
     ...
     $                ( JINC.GE.MIN( N_B, NNB ) )
            IF( .NOT.ISVALID ) THEN
               ...
               IF( .NOT.BSIZEQUERY ) THEN
                  CALL PXERBLA( ICONTXT, 'PFDGEMM', 14 )
               ENDIF
               RETURN
            ENDIF
*
...
*
               CALL DLAREAD( IODEV, M_B, N_B, IIB, JJB, B, 1, 1,
     $                       DESCBTMP, INFO )
               CALL ASSERT( INFO.EQ.0,
     $                      '** PFxGEMM: LAREAD returns info = ', INFO )
               CALL PDGEMM( TRANSA, TRANSB, M, N, K, ALPHA, A, IIA, JJA,
...
*
            IF( BETA.NE.ONE ) THEN
               DO 50 JJ = 1, N
                  CALL PDSCAL( M, BETA, C, IC, ( JC-1 )+JJ, DESCC, 1 )
   50          CONTINUE
   60          CONTINUE
            ENDIF
            LBETA = ONE
...
*
     ...
     $                            '** PFxGEMM: descinit returns info ',
     $                            INFO )
                     CALL DLAREAD( IODEV, ISIZE, JSIZE, IIB, JJB, B,
...
*
                  ...
                  IF( .NOT.ASIZEQUERY ) THEN
                     CALL PXERBLA( ICONTXT, 'PFDGEMM', 10 )
                  ENDIF
                  RETURN
               ENDIF
*
...
*
     ...
     $                         '** PFxGEMM: descinit returns info = ',
     $                         INFO )
                  CALL DLAREAD( IODEV, M_A, N_A, IA, JA, A, 1, 1,
     $                          DESCATMP, INFO )
                  CALL ASSERT( INFO.EQ.0,
     $                         '** PFxGEMM: LAREAD returns info = ',
     $                         INFO )
                  CALL PDGEMM( TRANSA, TRANSB, M, N, K, ALPHA, A, 1, 1,
...
*
               IF( BETA.NE.ONE ) THEN
                  DO 110 JJ = 1, N
                     CALL PDSCAL( M, BETA, C, IC, ( JC-1 )+JJ, DESCC,
     $                            1 )
  110             CONTINUE
  120             CONTINUE
               ENDIF
...
*
     ...
     $                            '** PFxGEMM: descinit returns info = '
     $                               , INFO )
                        CALL DLAREAD( IODEV, ISIZE, JSIZE, IIA, JJA, A,
     $                                IOFF, JOFF, DESCATMP, INFO )
                        CALL ASSERT( INFO.EQ.0,
     $                               '** PFxGEMM: LAREAD returns info ',
     $                               INFO )
                        CALL PDGEMM( TRANSA, TRANSB, MM, NN, KK, ALPHA,
