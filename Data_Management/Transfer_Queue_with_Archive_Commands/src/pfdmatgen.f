      SUBROUTINE PFDMATGEN( IODEV, ICTXT, AFORM, DIAG, M, N, MB, NB, A,
...
*     .. External Subroutines ..
      EXTERNAL           ASSERT, BLACS_GRIDINFO, DESCINIT, DLAWRITE,
     $                   IGSUM2D, INFOG2L, LAIO_INFO, PDMATGEN,
     $                   PFMAX2SIZE, PXERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( MB.NE.NB ) THEN
            INFO = -7
            CALL PXERBLA( ICTXT, 'PFDMATGEN', 7 )
            RETURN
         ENDIF
         MM = ILCM( MMB, NNB )
         NN = MM
...
*
   ...
   40    CONTINUE
     ...
     $                 ( MAX( II, JJ )-1 )*MAX( JJSIZE, IISIZE )
            ...
            IF( HASWORK ) THEN
               CALL PDMATGEN( ICTXT, LAFORM, LDIAG, ISIZE, JSIZE, MB,
...
*
 ...
 9999                      FORMAT( 'PFMATGEN: ', 'myprow,mypcol ',
   ...
   60             CONTINUE
               ENDIF
               CALL DLAWRITE( IODEV, ISIZE, JSIZE, ISTART, JSTART, A, 1,
     $                        1, DESCA, INFO )
