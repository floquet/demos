      SUBROUTINE PFZGETRF(M,N,A,IA,JA,DESCA,IPIV,INFO,IAM,counter)
*
*
...
*     ..
*     .. Scalar Arguments ..
      INTEGER            IA, INFO, JA, M, N, IAM, counter
*     ..
*     .. Array Arguments ..
...
*
     ...
     $                     MYPCOL )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCA( CTXT_ ), 'PFZGETRF', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
     ...
     $                     MYPCOL )
      ...
      IF( DESCA( SIZE_ ).LT.ANEED ) THEN
         ...
         RETURN
      ENDIF
      CALL PFZGETF2( M, N, A, IA, JA, DESCA, IPIV, INFO, IAM, counter )
      A( 1 ) = ANEED
      RETURN
      END
