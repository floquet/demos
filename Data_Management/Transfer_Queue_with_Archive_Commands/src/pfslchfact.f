      SUBROUTINE PFSLCHFACT( IODEV, M, IC, JC, WORK, LWORK, INFO )
*
*
...
*
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICONTXT, 'PFSLCHFACT', -INFO )
         ENDIF
         RETURN
      ENDIF
*
...
*
         LINFO = 0
         CALL PSPOTRF( 'Lower', JSIZEB, WORK( IP_BMAT ), 1, 1, DESCB,
     $                 LINFO )
         IF( LINFO.GE.1 ) THEN
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PSTRSM( 'Rightside', 'LowerTriangular', 'T',
...
*
            LINFO = 0
            CALL PSPOTRF( 'Lower', ISIZE, WORK( IP_BMAT ), ISTART,
     $                    ISTART, DESCB, LINFO )
            IF( LINFO.GE.1 ) THEN
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PSSYRK( 'Lower', 'NoTranspose', NN, KK, ALPHA,
     $                      WORK( IP_BMAT ), ( IEND+1 ), ISTART, DESCB,
...
*
            ...
            IF( HASWORK ) THEN
               CALL PSGEMM( 'NoTranspose', 'T', MM, NN, KK, ALPHA,
