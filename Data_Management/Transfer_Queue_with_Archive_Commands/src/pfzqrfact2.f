      SUBROUTINE PFZQRFACT2( IODEV, M, N, CWORK, IC, JC, DESCC, TAUC,
     $                       WORK, LWORK )
*
*
...
*     .. Array Arguments ..
      INTEGER            DESCC( FLEN_ )
      COMPLEX*16         CWORK( * ), TAUC( * ), WORK( LWORK )
*     ..
*     .. Local Scalars ..
...
*     .. External Subroutines ..
     ...
     $                   PFMAXSIZE, PFZCOPYTAU, PZGEQRF, PZUNMQR, ZFILL,
     $                   ZGEBR2D, ZGEBS2D, ZLAREAD, ZLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $             INFO )
      ICPOS = ( IC-1 ) + 1
      JCPOS = ( JC-1 ) + 1
      CALL ZLAREAD( IODEV, M, JSIZEB, ICPOS, JCPOS, CWORK( IP_BMAT ), 1,
     $              1, DESCB, INFO )
      CALL ASSERT( INFO.EQ.0, 'PFxQRFACT2 : LAREAD returns info ',
     $             INFO )
      LINFO = 0
      CALL PZGEQRF( M, JSIZEB, CWORK( IP_BMAT ), 1, 1, DESCB,
...
*
      ...
      IF( .NOT.ALL_INVOLVED ) THEN
         ...
         IF( HASWORK ) THEN
            IF( MYPROW.EQ.DESCB( RSRC_ ) ) THEN
               CALL ZGEBS2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ, 1,
     $                       WORK( IP_TAUB ), LOCQ )
            ELSE
               CALL ZGEBR2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ, 1,
     ...
     $                       MYPCOL )
      ...
      ENDIF
      JCPOS = ( JC-1 ) + JSTARTB
      CALL PFZCOPYTAU( JSIZEB, 1, WORK( IP_TAUB ), DESCB, JCPOS, TAUC,
     $                 DESCC )
      ...
      ICPOS = ( IC-1 ) + 1
      JCPOS = ( JC-1 ) + 1
      CALL ZLAWRITE( IODEV, M, JSIZEB, ICPOS, JCPOS, CWORK( IP_BMAT ),
     $               1, 1, DESCB, INFO )
...
*
         ...
         JCPOS = ( JC-1 ) + JSTARTB
         INFO = 0
         CALL ZLAREAD( IODEV, M, JSIZEB, ICPOS, JCPOS, CWORK( IP_BMAT ),
     $                 1, 1, DESCB, INFO )
...
*
     ...
     $                   JSTARTA )
            ...
            INFO = 0
            CALL ZLAREAD( IODEV, ISIZEA, JSIZEA, ICPOS, JCPOS,
     $                    CWORK( IP_AMAT ), IAPOS, JAPOS, DESCA, INFO )
            CALL ASSERT( INFO.EQ.0,
     $                   'PFxQRFACT2 : laread for A returns info ',
     $                   INFO )
            CALL ZFILL( TAUA_DIM, DZERO, WORK( IP_TAUA ), 1 )
            CALL PFZCOPYTAU( JSIZEA, JCPOS, TAUC, DESCC, JAPOS,
     $                       WORK( IP_TAUA ), DESCA )
*
...
*
     ...
     $                ( JSIZEB.GE.1 )
            IF( HASWORK ) THEN
               ...
               JBPOS = 1
               LINFO = 0
               CALL PZUNMQR( 'Leftside', 'C', MM, NN, KK,
...
*
         ...
         IF( HASWORK ) THEN
            ...
            JBPOS = 1
            LINFO = 0
            CALL PZGEQRF( MM, NN, CWORK( IP_BMAT ), IBPOS, JBPOS, DESCB,
     $                    WORK( IP_TAUB ), WORK( IFREE ), NFREE, LINFO )
            QR_NEED = MAX( QR_NEED, INT( WORK( IFREE ) ) )
            IF( LINFO.NE.0 ) THEN
               IF( LINFO.GE.1 ) THEN
...
*
     ...
     $                    NPROW )
                  IF( MYPROW.EQ.IROWB ) THEN
                     CALL ZGEBS2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ,
     $                             1, WORK( IP_TAUB ), LOCQ )
                  ELSE
                     CALL ZGEBR2D( DESCB( CTXT_ ), 'Column', 'I', LOCQ,
     ...
     $                             MYPCOL )
            ...
            ENDIF
            JCPOS = ( JC-1 ) + JSTARTB
            CALL PFZCOPYTAU( JSIZEB, 1, WORK( IP_TAUB ), DESCB, JCPOS,
     $                       TAUC, DESCC )
         ENDIF
*
*            Write out panel B.
*
         ...
         JCPOS = ( JC-1 ) + JSTARTB
         INFO = 0
         CALL ZLAWRITE( IODEV, M, JSIZEB, ICPOS, JCPOS,
