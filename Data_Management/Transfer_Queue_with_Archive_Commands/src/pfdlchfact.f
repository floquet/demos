      SUBROUTINE PFDLCHFACT( IODEV, M, IC, JC, WORK, LWORK, INFO )
*
*
...
*
      ...
      IF( LWORK.LT.INEED ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICONTXT, 'PFDLCHFACT', -INFO )
         ENDIF
         RETURN
      ENDIF
*
...
*
         LINFO = 0
         CALL PDPOTRF( 'Lower', JSIZEB, WORK( IP_BMAT ), 1, 1, DESCB,
     $                 LINFO )
         IF( LINFO.GE.1 ) THEN
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PDTRSM( 'Rightside', 'LowerTriangular', 'T',
...
*
            LINFO = 0
            CALL PDPOTRF( 'Lower', ISIZE, WORK( IP_BMAT ), ISTART,
     $                    ISTART, DESCB, LINFO )
            IF( LINFO.GE.1 ) THEN
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PDSYRK( 'Lower', 'NoTranspose', NN, KK, ALPHA,
     $                      WORK( IP_BMAT ), ( IEND+1 ), ISTART, DESCB,
...
*
            ...
            IF( HASWORK ) THEN
               CALL PDGEMM( 'NoTranspose', 'T', MM, NN, KK, ALPHA,
