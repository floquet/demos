      SUBROUTINE PFZGETF2(M,N,CWORK,IC,JC,DESCC,IPIV,INFO,IAM,counter)
*
*
...
*     ..
*     .. Scalar Arguments ..
      INTEGER            IC, INFO, JC, M, N, IAM, counter
*     ..
*     .. Array Arguments ..
...
*     .. External Subroutines ..
     ...
     $                   INFOG1L, LAIO_INFO, PCHK1MAT, PFMAXSIZE,
     $                   PXERBLA, PZGEMM, PZGETRF, PZLAPIV, PZTRSM,
     $                   ZLAREAD, ZLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*     .. Executable Statements ..
     ...
     $                     MYPCOL )
      ...
      IF( INFO.NE.0 ) THEN
         CALL PXERBLA( DESCC( CTXT_ ), 'PFZGETF2', 6 )
         RETURN
      ENDIF
      IDUMMY = 1
...
*
      INFO = 0
      CALL ZLAREAD( IODEV, M, NCOLB, IC, JC, CWORK( IP_BMAT ), 1, 1,
     $              DESCB, INFO )
...
*
      IINFO = 0
      CALL PZGETRF( M, NCOLB, CWORK( IP_BMAT ), 1, 1, DESCB,
     $              IPIV( IP_BPIV ), IINFO )
      IF( IINFO.GT.0 ) THEN
*
...
*
      INFO = 0
      CALL ZLAWRITE( IODEV, M, NCOLB, IC, JC, CWORK( IP_BMAT ), 1, 1,
     $               DESCB, INFO )
      CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAWRITE returns info = ',
     $             INFO )
      IF( JENDB.EQ.N ) THEN
*
*         Whole matrix fits in core.
*         All done.
*
         IF (IAM .EQ. 0) print*, "Whole matrix fits in core."
         INFO = 0
         LASTINDEX = M
         GOTO 90
      ENDIF
...
*
      ...
      IF( ANEED+BNEED.GT.LWORK ) THEN
         ...
         IF( .NOT.LWORKQUERY ) THEN
            CALL PXERBLA( ICONTXT, 'PFZGETF2', 6 )
         ENDIF
         LASTINDEX = ( JSTARTB-1 )
         GOTO 90
      ENDIF
...
*
         IIC = ( IC-1 ) + IC
         JJC = ( JC-1 ) + JSTARTB
         CALL ZLAREAD( IODEV, M, JSIZEB, IIC, JJC, CWORK( IP_BMAT ), 1,
     $                 1, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAREAD returns info = ',
     $                INFO )
*
...
*
         ...
         IF( HASWORK ) THEN
            CALL PZLAPIV( 'Forward', 'Rowise', 'Columwise', MM, NN,
...
*
               ...
               HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
               IF( HASWORK ) THEN
                  CALL PZLAPIV( 'Forward', 'Rowwise', 'Columnwise', MM,
...
*
                  ...
                  HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
                  IF( HASWORK ) THEN
                     CALL PZLAPIV( 'Forward', 'Rowwise', 'Columnwise',
...
*
                  ...
                  HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
                  IF( HASWORK ) THEN
                     CALL PZTRSM( 'Leftside', 'LowerTriangular',
     ...
     $                      ( KK.GE.1 )
                  IF( HASWORK ) THEN
                     ALPHA = -ONE
                     BETA = ONE
                     CALL PZGEMM( 'Notrans', 'NoTrans', MM, NN, KK,
     $                            ALPHA, CWORK( IP_AMAT ), IA, JA,
     $                            DESCA, CWORK( IP_BMAT ), IB, JB,
...
*
               ...
               IF( HASWORK ) THEN
                  ALPHA = -ONE
                  BETA = ONE
                  CALL PZGEMM( 'Notrans', 'NoTrans', MM, NN, KK, ALPHA,
...
*
         ...
         IF( HASWORK ) THEN
            ...
            JB = 1
            counter = counter + 1
            IF (IAM .EQ. 0 .and. counter .eq. 1) then
               print*, "Matrix is out of core."
            endif
            CALL PZGETRF( MM, NN, CWORK( IP_BMAT ), IB, JB, DESCB,
     $                    IPIV( IP_BPIV ), IINFO )
            IF( IINFO.GT.0 ) THEN
*
...
*
         IIC = ( IC-1 ) + IC
         JJC = ( JC-1 ) + JSTARTB
         CALL ZLAWRITE( IODEV, M, JSIZEB, IIC, JJC, CWORK( IP_BMAT ), 1,
     $                  1, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAWRITE returns info = '
     $                , INFO )
         JSTARTB = JENDB + 1
   ...
   70 CONTINUE
      IF( JSTARTB.LE.MINMN ) THEN
         ...
         IF( HASWORK ) THEN
            ...
            MM = M - ISTARTB + 1
            NN = JSIZEB
            CALL ZLAREAD( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ),
     ...
     $                    IPIV( IP_BPIV ), IB, 1, DESCIPIV, IDUMMY )
            MM = M - ISTARTB + 1
            NN = JSIZEB
            CALL ZLAWRITE( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ),
     $                     IB, JB, DESCB, INFO )
            CALL ASSERT( INFO.EQ.0,
...
*
     ...
     $               DESCB( CTXT_ ), LDPIV, INFO )
      ...
      IF( HASWORK ) THEN
         ...
         JB = 1
         MM = M - ISTARTB + 1
         CALL ZLAREAD( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ), IB,
     $                 JB, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAREAD returns info = ',
     $                INFO )
         MM = MIN( M, N ) - ISTARTB + 1
     ...
     $                 IB, 1, DESCIPIV, IDUMMY )
         MM = M - ISTARTB + 1
         CALL ZLAWRITE( IODEV, MM, NN, IIC, JJC, CWORK( IP_BMAT ), IB,
     $                  JB, DESCB, INFO )
         CALL ASSERT( INFO.EQ.0, '**  PFxGETF2: LAWRITE returns info = '
     $                , INFO )
      ENDIF
