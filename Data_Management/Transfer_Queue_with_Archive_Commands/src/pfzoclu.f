      SUBROUTINE PFZOCLU( IODEV, NNB, M, N, IC, JC, DESCC, IPIV, LDPIV,
     $                    WORK, LWORK, INFO )
*
*
...
*     .. External Subroutines ..
     ...
     $                   PZGEMM, PZGETRF, PZLAPIV, PZTRSM, ZLAREAD,
     $                   ZLAWRITE
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL ZLAREAD( IODEV, M, GNCOLB, IC, JC, WORK( IP_BMAT ), 1, 1,
     $              DESCB, INFO )
      CALL ASSERT( INFO.EQ.0, '** PFOCLU: dlread returns info ', INFO )
      CALL PZGETRF( M, GNCOLB, WORK( IP_BMAT ), 1, 1, DESCB, IPIV,
     $              INFO )
...
*
         ...
         IF( HASWORK ) THEN
            CALL PZLAPIV( 'B', 'R', 'C', MM, NN, WORK( IP_BMAT ), 1, 1,
     $                    DESCB, IPIV, 1, 1, DESCIPIV, IDUM )
         ENDIF
      ENDIF
      CALL ZLAWRITE( IODEV, M, GNCOLB, IC, JC, WORK( IP_BMAT ), 1, 1,
     $               DESCB, INFO )
...
*
         ICPOS = ( IC-1 ) + 1
         JCPOS = ( JC-1 ) + JSTARTB
         CALL ZLAREAD( IODEV, M, JSIZEB, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                 1, 1, DESCB, INFO )
...
*
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL PZLAPIV( 'F', 'R', 'C', MM, NN, WORK( IP_BMAT ), 1, 1,
     $                    DESCB, IPIV, 1, 1, DESCIPIV, IDUM )
         ENDIF
*
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL ZLAREAD( IODEV, M, JSIZEA, ICPOS, JCPOS,
...
*
            ...
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PZLAPIV( 'F', 'R', 'C', MM, NN, WORK( IP_AMAT ), 1,
...
*
            HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
            IF( HASWORK ) THEN
               CALL PZTRSM( 'Left', 'Lower', 'No transpose', 'Unit', MM,
...
*
            ...
            IF( HASWORK ) THEN
               CALL PZGEMM( 'No transpose', 'No transpose', MM, NN, KK,
...
* ========================================
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL PZGETRF( MM, NN, WORK( IP_BMAT ), IB, JB, DESCB, IPIV,
     $                    INFO )
...
* ---------------------------
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL ZLAWRITE( IODEV, MM, NN, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                     1, 1, DESCB, INFO )
...
*
         ...
         HASWORK = ( MM.GE.1 ) .AND. ( NN.GE.1 )
         IF( HASWORK ) THEN
            CALL ZLAREAD( IODEV, MM, NN, ICPOS, JCPOS, WORK( IP_BMAT ),
     $                    1, 1, DESCB, INFO )
            CALL ASSERT( INFO.EQ.0, '** PFOCLU: laread returns info ',
     $                   INFO )
         ENDIF
